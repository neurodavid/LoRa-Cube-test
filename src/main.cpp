 ...
 * This sketch has been modifyed by Pär Davidsson 2024
 * One problem is that if it dosent get a respond it stops sending. 
 * This means that you have to reset one of the module to get it to start send/recive again
 * It is a problem if you have no physical connection to the boards.
 * Has been tested to send 1km with a noted reciver signal at -107 dB
 * and a send signal issi of -95 dB (send/recive from location maskinhallen)
 * 
 */
